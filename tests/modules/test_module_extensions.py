...
from tlsmate.ext import ServerMalfunction
...
def test_sni_wrong_length():
    ...
    with pytest.raises(ServerMalfunction, match="extension length incorrect"):
        sni, offset = ext.Extension.deserialize(data, 0)


def test_sni_wrong_name_type():
    data = bytes.fromhex("00 00 00 0e 00 0c 01 00 09 6c 6f 63 61 6c 68 6f 73 74")
    with pytest.raises(ServerMalfunction, match="host_name not present"):
        sni, offset = ext.Extension.deserialize(data, 0)
